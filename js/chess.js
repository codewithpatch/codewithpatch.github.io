...
class ChessBoard {
    ...
    static selectBoardElement() {
        return document.getElementsByClassName('chess-board')[0];
    }
    static generateBoardNotations() {
...
class ChessPosition {
    ...
    static coordinatesToNotation(coordinates) {
        let column = String.fromCharCode(coordinates[0] + 65);
        let row = String(coordinates[1] + 1);
        return column + row;
    }
    static notationToCoordinates(notation) {
        let x = notation[0].charCodeAt(0) - 65;
        let y = Number(notation[1]);
        return [x, y];
    }
    static fromHTMLPieceElement(element) {
...
class ChessPiece {
    constructor(name, pieceNumber, color, image, moveDescription) {
...
class Bishop extends ChessPiece {
    constructor(pieceNumber, color, image) {
